...
export type AnyModel = CookieJar | Environment | Folder | GrpcConnection | GrpcEvent | GrpcRequest | HttpRequest | HttpResponse | Plugin | Settings | KeyValue | SyncState | Workspace;
...
export type SyncState = { model: "sync_state", id: string, workspaceId: string, createdAt: string, updatedAt: string, dirty: boolean, modelId: string, lastFlush: SyncStateFlushState | null, path: string, };

export type SyncStateFlushState = { time: string, checksum: string, };

export type UpdateSource = "sync" | "window" | "plugin" | "background";

export type Workspace = { model: "workspace", id: string, createdAt: string, updatedAt: string, name: string, description: string, variables: Array<EnvironmentVariable>, settingValidateCertificates: boolean, settingFollowRedirects: boolean, settingRequestTimeout: number, settingSyncDir: string | null, };
