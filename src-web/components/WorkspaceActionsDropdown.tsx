...
export const WorkspaceActionsDropdown = memo(function WorkspaceActionsDropdown({
  ...
  const openCloneGitRepositoryDialog = useCallback(() => {
    showDialog({
      ...
      size: "md",
      title: "Клонировать Git-репозиторий",
  ...
  const { workspaceItems, itemsAfter, itemsBefore } = useMemo<{
    ...
    const itemsBefore: DropdownItem[] = [
      {
        ...
        submenu: [
          ...
          },
          {
            ...
            onSelect: async () => {
              const dir = await open({
                title: "Выберите каталог рабочего пространства",
                directory: true,
                multiple: false,
              });
          ...
          {
            label: "Клонировать Git-репозиторий",
    ...
    const itemsAfter: DropdownItem[] = [
      ...
      {
        label: "Настройки рабочего пространства",
      ...
      {
        label: "Очистить историю отправок",
        color: "warning",
