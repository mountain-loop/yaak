...
export function FilterHistoryDropdown({
  ...
  const historyItems = useMemo<DropdownItem[]>(() => {
          ...
          <div className="flex items-center gap-0.5">
            <span
              className={classNames(
                ...
                item.pinned
                  ? "bg-notice/20 text-notice opacity-0 group-hover:opacity-100 group-focus:opacity-100"
                  : "text-text-subtlest group-hover:text-primary group-focus:text-primary hover:!text-primary hover:bg-primary/10 opacity-0 group-hover:opacity-100 group-focus:opacity-100",
              )}
              title={item.pinned ? "Unpin from history" : "Pin to history"}
              onClick={(e) => {
                e.stopPropagation();
    ...
    // Add grouped items
    const groups = [
      { label: 'Pinned', items: groupedHistory.pinned, showDate: false },
    ...
    return items;
  }, [history, currentValue, onSelect, onRemove, onClearAll, onTogglePin, groupedHistory]);
