...
/// and streams back what came back, and with `--serve-web` hands the browser the app as well.
/// Nothing is written to disk or a database.
#[derive(Parser, Debug, Clone)]
#[command(name = "yaak-server", version, about, long_about = None)]
pub struct Config {
    /// Address to listen on. 127.0.0.1 for a local instance; 0.0.0.0 inside a container.
    #[arg(long, env = "YAAK_SERVER_BIND", default_value = "127.0.0.1:9227")]
    pub bind: SocketAddr,
    ...
    #[arg(long, env = "YAAK_SERVER_SERVE_WEB", value_name = "DIR")]
    ...
    #[arg(long, env = "YAAK_SERVER_ALLOW_PRIVATE_NETWORKS", default_value_t = false)]
    ...
    #[arg(
        long,
        env = "YAAK_SERVER_ALLOWED_ORIGINS",
        default_value = "*",
        value_delimiter = ','
    )]
    ...
    #[arg(long, env = "YAAK_SERVER_MAX_REQUEST_BYTES", default_value_t = 16 * 1024 * 1024)]
    pub max_request_bytes: usize,

    /// Largest upstream response body the server will relay before cutting the send off.
    #[arg(long, env = "YAAK_SERVER_MAX_RESPONSE_BYTES", default_value_t = 64 * 1024 * 1024)]
    pub max_response_bytes: usize,

    /// Ceiling on a send's timeout, in seconds. A request asking for longer (or for no timeout)
    /// gets this instead.
    #[arg(long, env = "YAAK_SERVER_MAX_TIMEOUT_SECS", default_value_t = 60)]
    ...
    #[arg(long, env = "YAAK_SERVER_RATE_LIMIT_PER_MINUTE", default_value_t = 120)]
    pub rate_limit_per_minute: u32,

    /// Sends in flight at once across all clients.
    #[arg(long, env = "YAAK_SERVER_MAX_CONCURRENT", default_value_t = 256)]
    ...
    /// past the rate limit.
    #[arg(long, env = "YAAK_SERVER_TRUST_FORWARDED_FOR", default_value_t = false)]
